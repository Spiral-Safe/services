...
function authenticate(config: ServiceConfig) {
  return (
    ...
    if (!authorization.startsWith("Bearer ")) {
      next(
        new HTTPError(401, "unauthorized", "Bearer authentication is required"),
      );
      return;
    }
...
function requireSigningRequest(
  ...
  return { operation: operation as Operation, payload };
}
...
export function createApp(
  ...
  app.post(
    ...
    asyncRoute(async (req, res) => {
      res.status(200).json(
        await vault.post("users", {
          ...identityBody(req),
          ceremonyId: requireCeremonyID(req),
  ...
  app.post(
    "/check",
    asyncRoute(async (req, res) => {
      res.status(200).json(await vault.post("check", identityBody(req)));
    }),
  );
  app.post(
    "/signin",
    asyncRoute(async (req, res) => {
      ...
      res
        .status(200)
        .json(await vault.post("auth", { ...identityBody(req), ...signing }));
    }),
  );
  app.post(
    "/complete",
